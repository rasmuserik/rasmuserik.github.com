...
bundler.module('scripts/timelog.js','var window=require("window"),storage=window.localStorage,$=require("zquery"),_=require("underscore"),util=require("util"),account,lastTime,recentLog,tasks,changes;function scriptString(obj){var result=\x27<script src="https://rasmuserik-timelog.appspot.com/?callback=timelogResponseFromServer&name=\x27+account+"&since="+lastTime;return obj&&(result+="&id="+obj.id+"&value="+obj.value),result+=\x27"></script>\x27,result}function update(){var elem=!1;Object.keys(changes).length>0&&(elem=changes[Object.keys(changes)[0]],console.log("WARNING:",elem,changes)),window.document.write(scriptString(elem))}function setCurrent(name){var now=+(new Date);changes[now]={id:now,value:name},update()}exports.update=update,window.timelogResponseFromServer=function(eventArr){eventArr.forEach(function(elem){recentLog[elem.id]=elem,lastTime=Math.max(lastTime,elem.timestamp);var task=tasks[elem.value]||{value:elem.value,earliest:elem.id,latest:0};task.latest=Math.max(task.latest,elem.id),tasks[elem.value]=task,changes[elem.id]&&delete changes[elem.id]}),(eventArr.length===1e3||Object.keys(changes).length>0)&&update(),save()};function save(){storage.setItem("timelogAccount",account),storage.setItem("timelogLastTime",JSON.stringify(lastTime)),storage.setItem("timelogRecentLog",JSON.stringify(recentLog)),storage.setItem("timelogTasks",JSON.stringify(tasks)),storage.setItem("timelogChanges",JSON.stringify(changes))}function openAccount(){account=storage.getItem("timelogAccount")||window.prompt("Please enter a name/passphrase for your timelog account. This should be a globally uniq secret name only you should know. Anybody knowing the name can acces the timelog account. If the name does not exist already, a new timelog will be created");if(!account)throw{error:"no timelog account"};storage.setItem("timelogAccount",account),lastTime=JSON.parse(storage.getItem("timelogLastTime")||"0"),recentLog=JSON.parse(storage.getItem("timelogRecentLog")||"{}"),tasks=JSON.parse(storage.getItem("timelogTasks")||"{}"),changes=JSON.parse(storage.getItem("timelogChanges")||"{}")}function getTaskList(n){var entries=_(tasks).values().sort(function(a,b){return b.latest-a.latest}).slice(0,n),result=[];return entries=entries.sort(function(a,b){return b.earliest-a.earliest}),_(entries).pluck("value")}function getCurrent(){var time=0,value="";return _(tasks).each(function(_,entry){entry.latest>time&&(value=entry.value,time=entry.latest)}),value}exports.main=function(){openAccount(),update(),console.log(getTaskList(16)),console.log(getCurrent())}');
