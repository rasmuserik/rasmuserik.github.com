...
bundler.module('scripts/util.js','var window=require("window"),$=require("zquery");exports.niceSingle=function(fn){var running=!1;return function(){if(running)return;running=!0,window.setTimeout(function(){fn(),running=!1},10)}},exports.windowHeight=function(){var height=$(window).height();if(height===356||height===208)height+=60;return height};var seed=1,pseudoRandom=exports.pseudoRandom=function(n){return seed=1664525*(n||seed)+1013904223|0};exports.colorHash=function(text){var result=0;for(var i=0;i<text.length;++i)result=pseudoRandom(result+text.charCodeAt(i));return"#"+((result|14737632)&16777215).toString(16)}');
...
bundler.module('scripts/fullbrows.js','var util=require("util"),$=require("zquery"),window=require("window"),Modernizr=require("modernizr"),relayoutFn,browsOpt;function relayout(){$("#content").css("position","absolute").css("left",0).css("top",1).css("overflow","hidden").css("width",$(window).width()).css("height",browsOpt.scrollable?"auto":util.windowHeight()),typeof relayoutFn=="function"&&relayoutFn(window.document.getElementById("content")),window.scrollTo(0,1)}var relayoutDelayed=util.niceSingle(relayout),init=exports.init=function(opt){browsOpt=opt||{},relayoutFn=browsOpt.callback,window.document.getElementById("content")||$("body").append(\x27<div id="content"></div>\x27),Modernizr.touch?$("body").append(\x27<div style="height:\x27+(Math.max(util.windowHeight(),$(window).width())+62)+\x27px;background-color: black;"></div>\x27):$("body").css("overflow","hidden"),$("body").css("background-color","black"),$(window).bind("resize",relayoutDelayed),$(window).bind("orientationchange",relayoutDelayed),relayout()}');
